...
class ConfiguradorNavegacion {
    static configurarBotonInicio() {
        document.querySelectorAll('a[href="./bienvenida"]').forEach(link => {
            link.addEventListener("click", (e) => {
                e.preventDefault();
                const nombreUsuario = Persona.obtenerNombre();
                window.location.href = nombreUsuario ? "productos.html" : "bienvenida";
            });
        });
    }
    ...
    static configurarBotonCerrarSesion() {
        document.querySelectorAll('a[href="./bienvenida"]').forEach(link => {
            link.addEventListener("click", (e) => {
                e.preventDefault();
                const nombreUsuario = Persona.obtenerNombre();
                nombreUsuario ? PersonaController.cerrarSesion() : window.location.href = "bienvenida";
            });
        });
    }
}
